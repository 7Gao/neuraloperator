...
train_loader, test_loaders = load_arz_1d(data_path=config.data.folder,
...
model.load_state_dict(torch.load("/home/qi/Workspace/github/neuraloperator/trained_model/arz_240229_2336.pt"))
...
for index in range(5):
    # Input x
    x = test_samples.x[index+200]
    # Ground-truth
    y = test_samples.y[index+200]
    # Model prediction
    out = model(x.to(device).unsqueeze(0))
    out = out.to('cpu').detach().numpy().reshape((2, resulution))
    y = y.reshape((2, resulution))

    ax[0, index].plot(x_axs, y[1], label='Ground-truth')
    ax[0, index].plot(x_axs, out[1], '--', label='Prediction')
    ax[0, index].set_title(f'Example {index+1}')

    ax[1, index].plot(x_axs, y[0], label='Ground-truth')
    ax[1, index].plot(x_axs, out[0], '--', label='Prediction')
    ax[1, index].set_xlabel('$x$')

    ax[0, index].set_ylim([0, 1])
    ax[1, index].set_ylim([0, 1])

    ax[0, index].legend()
    ax[1, index].legend()
    
    if index == 0:
        ax[0, index].set_ylabel('$\\rho$')
        ax[1, index].set_ylabel('u')
